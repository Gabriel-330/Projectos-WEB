...
$anoLectico = "$anoAnterior/$anoAtual"; // Ex: 2024/2025
$mapaNotas = [];
...
foreach ($notas as $nota) {
    ...
    foreach ($matriculas as $matricula) {
        $disciplinas = $disciplinaDAO->listarTodosCursoClasse($nota->getNomeCurso(), $matricula->getClasseMatricula());
    }
...
$disciplinas = array_keys($mapaNotas);
...
{
    if ($nota === '-' || $nota === null) {
        return "<span style='color: #6c757d;'>-</span>"; // cinza (muted)
    }

    $cor = ($nota >= 10) ? "#0d6efd" : "#dc3545"; // azul bootstrap / vermelho bootstrap
