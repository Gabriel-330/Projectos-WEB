    ...
    public function retornarIdPorUtilizador($id)
    {
        try {
            $sql = "SELECT a.*, c.nomeCurso, u.nomeUtilizador, t.nomeTurma
                ...
                LIMIT 1"; // Adicionado LIMIT para garantir apenas 1 resultado
            $stmt = $this->conexao->prepare($sql);
            $stmt->bindValue(":id", $id, PDO::PARAM_INT); // Especificado tipo
            $stmt->execute();
            ...
            // Idealmente usar um logger e não die() em produção
            die("Erro ao buscar aluno: " . $e->getMessage());
        }
    }
