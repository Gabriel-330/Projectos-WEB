...
class DisciplinaDAO
{
    ...
    {
        try {
            $stmt = $this->conexao->prepare("INSERT INTO disciplina (nomeDisciplina, idCurso,idProfessor) VALUES (?, ?,?)");
            $stmt->execute([
    ...
    {
        try {
            $sql = "UPDATE disciplina SET nomeDisciplina = :nome, idCurso = :idCurso, idProfessor =: idProfessor WHERE idDisciplina = :id";
    ...
    public function listarTodosCursoClasse($classe,$curso)
    {
        try {
            $sql = "SELECT d.*, c.nomeCurso, p.nomeProfessor
                FROM disciplina d
                INNER JOIN curso c ON d.idCurso = c.idCurso
                INNER JOIN professor p ON d.idProfessor = p.idProfessor
                WHERE c.idCurso = :curso AND d.classeDisciplina = :classe";
